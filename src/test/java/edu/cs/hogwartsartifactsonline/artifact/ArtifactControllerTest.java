...
//        when & then together
        this.mockMvc.perform(get
                        (this.baseUrl + "/artifacts/1250808601744904191")
...
//        given
        ...
        given(this.artifactService.findAllPagination(Mockito.any(Pageable.class))).willReturn(artifactPage);
...
//        when and then
        this.mockMvc.perform(get(this.baseUrl + "/artifacts/pagination").accept(MediaType.APPLICATION_JSON).params(requestParams))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
                .andExpect(jsonPath("$.message").value("Find All Success"))
                .andExpect(jsonPath("$.data.content").value(Matchers.hasSize(this.artifacts.size())))
...
//        given
        ArtifactDto artifactDto = new ArtifactDto(null,
                "Remember All",
                "A Remember All was a magical large marble-sized glass ball that contained smoke which turned red when its owner or user had forgotten something. It turned clear once whatever was forgotten was remembered.",
                "ImageUrl",
                null);
        ...
        savedArtifact.setName("Remember All");
        savedArtifact.setDescription("A Remember All was a magical large marble-sized glass ball that contained smoke which turned red when its owner or user had forgotten something. It turned clear once whatever was forgotten was remembered.");
